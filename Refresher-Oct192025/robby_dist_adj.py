...
# --- FIXED DISTANCE FUNCTION (with filtering) ---
def measure_distance():
    readings = []
    for _ in range(3):  # take 3 readings to filter noise
        trigger.low()
        sleep_us(2)
        trigger.high()
        sleep_us(10)
        ...
        # only keep valid readings within reasonable range
        if 2 < distance < 400:
            readings.append(distance)
        sleep(0.01)  # small delay between samples

    if len(readings) == 0:
        return 999  # return a large number if no valid readings
    readings.sort()
    return readings[len(readings) // 2]  # return median value
